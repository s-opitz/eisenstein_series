...
class JordanDecomposition( SageObject):
    ...
    def signature(self):
        if self.genus_symbol() == '':
            return 0
        else:
            return self.__A.signature()
    ...
    def dimension_vector_valued_mod_forms(self, weight=None, weights=[], verbose=False):
        r"""
        Return the dimension of the corresponding space of vector valued modular
        forms for the Weil representation using the formulas in [Bru]
        """
        if weight:
            weights = [weight]
        s = self.signature()
        for weight in weights:
            if weight <= 2:
                raise NotImplementedError
            if (Integer(2 * weight) + s) % 2 != 0:
                return 0, 0
            if (Integer(2 * weight) + s) % 4 != 0:
                raise NotImplementedError
        N = lcm(self.__A.level(), 24)
        C = CyclotomicField(N)
        z = C.gen()
        vals = self.values()
        vals2 = self.two_torsion_values()
        number_of_2torsions = Integer(0)
        for x in vals2.keys():
            number_of_2torsions += vals2[x]
        
        def gauss_sum(a):
            r"""
            If this quadratic module equals $D = (M,Q)$, return
            the Gauss sum of $D$ at $a$, i.e. return
            $$G(a,D)= \sum_{x\in M} \exp(2\pi i a Q(x)))$$
            computed naively by using the values method of the Jordan Decomposition.
            NOTE: The cyclotomic field will be chosen to contain sqrt(|D|) and
            such that G(1,D), G(2,D) and G(-3,D) can be computed within it.
            """
            res = 0
            for x in vals.keys():
                res += vals[x] * z**(a*x*N)
        ...
        D = self.__A.order()
        d = (D + number_of_2torsions)/Integer(2)
        g1 = gauss_sum(1)
        g2 = gauss_sum(2)
        g3 = gauss_sum(-3)
        if verbose:
            print "computing square roots in C=", C
        sqrt3 = 2 * z**Integer(N/Integer(12)) - z**Integer(N/Integer(4))
        #if verbose:
        #    print "sqrt3 done"#, sqrt3, CC(sqrt3), sqrt3**2
        #    print "computing sqrt(" + str(D) + ")"
        if CC(sqrt3).real_part() < 0:
            sqrt3 *= -1
        if sqrt3**2 != 3:
            raise RuntimeError, "The computation of sqrt(3) seems faulty"
        z8 = z**Integer(N / Integer(8))
        sqrtD = g1
        for k in range(8):
            if sqrtD**2 == D:
                break
                ...
                sqrtD *= z8
        if CC(sqrtD).real_part() < 0:
            sqrtD *= -1
        if sqrtD**2 != D:
            raise RuntimeError, "The computation of sqrt(|D|) seems faulty"
        if verbose:
            #print "sqrt(" + str(D) + ")=" + str(sqrtD), CC(sqrtD), sqrtD**2
            print "done with the square roots"
        alpha1realpart = (g2 + g2.conjugate())/Integer(2)
        alpha3 = 0
        for x in vals.keys():
            alpha3 += vals[x] * (-x - floor(-x))
        for x in vals2.keys():
            alpha3 += vals2[x] * (-x - floor(-x))
        alpha3 /= Integer(2)
        alpha4 = (vals[0] + vals2[0]) / Integer(2)
        dims = dict()
        for weight in weights:
            k = Integer(2 * weight)/Integer(2)
            alpha1 = d/Integer(4) - 1/(4 * sqrtD) * z**Integer((2*k + s)*N/Integer(8)) * alpha1realpart
            alpha2tilde = z**Integer((4*k+3*s-10)*N/Integer(24)) * (g1 + g3)
            alpha2realpart = (alpha2tilde + alpha2tilde.conjugate())/Integer(2)
            alpha2 = d/Integer(3) + 1/(3 * sqrt3 * sqrtD)*alpha2realpart
            dim = d + d * k / Integer(12) - alpha1 - alpha2 - alpha3
            if verbose and False:
                ...
                print "g2:", g2
                print "g3:", g3
                print "sqrt3", sqrt3, CC(sqrt3).real_part()
                print "sqrtD", sqrtD, CC(sqrtD).real_part()
                print "alpha1:", alpha1
                print "alpha2:", alpha2
                print "alpha3:", alpha3
                print "alpha4:", alpha4
        ...
        if len(dims.keys()) == 1:
            dims = dims[dims.keys()[0]]
            
        return dims, Integer(alpha4)
        
    def dimension_vector_valued_mod_forms__numeric(self, weight=None, weights=[], verbose=False):
        r"""
        Return the dimension of the corresponding space of vector valued modular
        forms for the Weil representation using the formulas in [Bru]
        """
        ...
        #if verbose:
        #    print "computing square roots in C=", C
        sqrt3 = C(3).sqrt()#= 2 * z**Integer(N/Integer(12)) - z**Integer(N/Integer(4))
        #if verbose:
        #    print "sqrt3 done"#, sqrt3, CC(sqrt3), sqrt3**2
        #    print "computing sqrt(" + str(D) + ")"
        if sqrt3.real_part() < 0:
            sqrt3 *= -1
        #if sqrt3**2 != 3:
        #    raise RuntimeError, "The computation of sqrt(3) seems faulty"
        #z8 = z**Integer(N / Integer(8))
        ...
        for weight in weights:
            k = Integer(2 * weight)/Integer(2)
            alpha1 = d/Integer(4) - 1/(4 * sqrtD) * exp((2*k + s)/Integer(4)*pi*i) * alpha1realpart
            alpha2tilde = exp((4*k+3*s-10)/Integer(12)*pi*i) * (g1 + g3)
            alpha2realpart = (alpha2tilde + alpha2tilde.conjugate())/Integer(2)
            alpha2 = d/Integer(3) + 1/(3 * sqrt3 * sqrtD)*alpha2realpart
            dim = d + d * k / Integer(12) - alpha1 - alpha2 - alpha3
            if verbose and False:
                ...
                print "alpha4:", alpha4
                print "dim:", dim
            dim_integral = dim.real_part().round()
            if 10**8 * (dim_integral - dim).abs() >= 1:
                raise RuntimeError, "The precision is not high enough"
            dims[k] = dim_integral #Integer(dim)
